...
- The Injuries agent uses retrieval-augmented generation over real injury/team-news text, plus a small fine-tuned classifier for injury severity, rather than relying only on structured availability fields.
...
- LLM calls are hosted through Microsoft Foundry (formerly Azure AI Foundry), with Prompt Shields guarding against both direct prompt injection and indirect/document-based injection — the latter is a real risk here, since the Injuries agent ingests external news text.
...
- **RAG**: vector store over injury/team-news text for the Injuries agent
